...
%  impact of the type of the source process (D, M, 1) and (U, M,1) on queuing ratio.
...
%%%
x=[1 2 ]
x = categorical({'D/M/1','U/M/1'})%'M/M/1'});
x = reordercats(x,{'D/M/1','U/M/1'})%'M/M/1'});
...
title('Effect of the type of the source process (D, M, 1) and (U, M,1) on Queuing Ratio');legend('95% CI','Lag-1 Autocorrelation');
xlabel('Systems');
